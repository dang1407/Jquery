...
var jq = $.noConflict();
var numberGroup = 0;
// jq(document).on('click', 'i',function(){
...
// jq(".gr-head-input").on('keydown', function(e){
//       if(e.keyCode == 13) {
//             let jqP = jq('<p>', {class: "gr-head"});
//             jqP.text(jq(this).text());
//             jq(this).html(jqP);
//       }
// })
...
jq(document).on("keypress", ".gr-head-input", function (event) {
  if (event.keyCode === 13) {
    ...
    if (jq(this).val() != "") jqNewP.text(jq(this).val() + "_20204720");
    else {
...
// Không để trống giá trị trường thông tin
// jq(document).on('blur', '.info-input', function(){
//       if(jq(this).val() == ''){
//             alert("Bạn không được để trống trường thông tin!");
//             jq(this).focus();
//             return;
...
// jq(document).on('dblclick', '.info-input', function(){
//       jq(this).focus();
// })
// jq(document).on('click', '.info-input', function(e){
//       e.preventDefault();
//       jq(this).blur();
// })

// Tạo PDF bằng jsPDF và html2canvas

// jq(document).on("click", ".get-pdf", function(){
//             var pdf = new jsPDF('p', 'pt', 'a4');
//             pdf.addHTML($(".main"), 15, 15, options, function() {
